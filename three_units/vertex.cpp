...
void Vertex :: computeSymOccPr(unsigned int card, unsigned int deg)
{
	...
	symOccPr[0] = p*p*p;
	symOccPr[1] = 3*p*p*(1-p);
	symOccPr[2] = 3*p*(1-p)*(1-p);
	symOccPr[3] = (1-p)*(1-p)*(1-p);
...
long double Vertex :: computeChiSqValue(const Node& qnode, unsigned int card)
{
	...
	unsigned int q_ng = qDegree*(qDegree-1)*(qDegree-2)/(3*2);	// Number of groups exhibited by query node
	...
	// Observed values
	// s0 = quadruplets with all labels from label_nomatch
	// s1 = quadruplets with two labels from label_nomatch and one from label_match
	// s2 = quadruplets with one label from label_nomatch and two from label_match
	// s3 = quadruplets with all labels from label_match

	#ifdef MICRO_DEBUG
		cout<<"label match and no match: "<<label_match<<" "<<label_nomatch<<endl;
	#endif // MICRO_DEBUG

	observed[0] = nCr(label_nomatch, 3);
	observed[1] = nCr(label_nomatch, 2) * nCr(label_match, 1);
	observed[2] = nCr(label_match, 2) * nCr(label_nomatch, 1);
	observed[3] = nCr(label_match, 3);
	...
	for(unsigned int i=0; i<NUM_OF_SYMBOLS; i++)
	{
		...
		cout<<"Probability (s0, s1, s2, s3):\t("<<getSymOccPr(0)<<", "<<getSymOccPr(1)<<", "<<getSymOccPr(2)<<","<<getSymOccPr(3)<<")\n";
		cout<<"Number of groups (triplets) of query node:\t"<<q_ng<<endl;
		cout<<"Expected (s0, s1, s2, s3):\t("<<getSymOccPr(0)*q_ng<<", "<<getSymOccPr(1)*q_ng<<", "<<getSymOccPr(2)*q_ng<<", "<<getSymOccPr(3)*q_ng<<")\n";
		cout<<"label match:\t"<<label_match<<"\tmismatch:\t"<<label_nomatch<<endl;
		cout<<"Observed (s0, s1, s2, s3):\t("<<observed[0]<<", "<<observed[1]<<", "<<observed[2]<<", "<<observed[3]<<")"<<endl;
		cout<<"******************\n";
	#endif // DEBUG
