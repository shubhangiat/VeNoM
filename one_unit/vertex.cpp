...
long double Vertex :: computeChiSqValue(const Node& qnode, unsigned int card)
{
	...
	unsigned int q_ng = qDegree;	// Number of groups exhibited by query node
	...
	observed[0] = label_nomatch;
	observed[1] = label_match;
	...
	for(unsigned int i=0; i<NUM_OF_SYMBOLS; i++)
	{
		...
		cout<<"\nDegree of query node:\t"<<qDegree<<endl;
		cout<<"Degree of target vertex:\t"<<getDegree()<<endl;
		cout<<"Probability (s0, s1):\t("<<getSymOccPr(0)<<", "<<getSymOccPr(1)<<")\n";
		cout<<"Number of groups (triplets) of query node:\t"<<q_ng<<endl;
		cout<<"Expected (s0, s1):\t("<<getSymOccPr(0)*q_ng<<", "<<getSymOccPr(1)*q_ng<<")\n";
		cout<<"label match:\t"<<label_match<<"\tmismatch:\t"<<label_nomatch<<endl;
		cout<<"Observed (s0, s1):\t("<<observed[0]<<", "<<observed[1]<<")"<<endl;
		cout<<"******************\n";
	#endif // DEBUG
