...
vector<vector<Vertex*> > Input :: getSubGraphs(const Query& qry)
{
	...
	const unordered_map<string, Node*> qryG = qry.getGraph_CRITICAL();
	...
	{	
		...
		vp ph_cand(vmc->second.first, vmc->first, vmc->second.second, qryG.at(vmc->second.first)->getDegree());
	...
	for(unsigned i=0; i<subgraph.size(); i++)   // For each candidate subgraph explore (grow) subgraph
	{
		...
		vp sh_vp(subgraph[i][0]->get_qVertexID(), subgraph[i][0]->getID(), subgraph[i][0]->getChiValue(subgraph[i][0]->get_qVertexID()), qryG.at(subgraph[i][0]->get_qVertexID())->getDegree());
		#ifdef DEBUG
			sh_vp.parent="INIT";
		#endif	// DEBUG
		...
		while( ((subgraph[i]).size() < max_vertex) && (!secondary_heap.empty()) )
		{
				...
				while(!copy_ph.empty())
				{
					cout<<"("<<copy_ph.top().getVID()<<", "<<copy_ph.top().getQID()<<", "
						<<copy_ph.top().getChiValue()<<", "<<copy_ph.top().getQDeg()<<") ";
					copy_ph.pop();
				}
			#endif // DEBUG
			#ifdef DMEASURE
			...
			#ifdef DEBUG
				cout<<"\nSH POP (cand): ("<<secondary_heap.top().getVID()<<", "
					<<secondary_heap.top().getQID()<<", "<<secondary_heap.top().getChiValue()
					<<", "<<secondary_heap.top().getQDeg()<<", "<<secondary_heap.top().parent
					<<")"<<endl;
			#endif // DEBUG
			...
			if(q_visited.find(cand->get_qVertexID())!=q_visited.end())
			{
				...
				{
					vp sh_cand(best_match_q, cand->getID(), best_chisq, qryG.at(best_match_q)->getDegree());
					#ifdef DEBUG
						sh_cand.parent = sh_top.parent;
						cout<<"SH PUSH: ("<<sh_cand.getVID()<<", "<<sh_cand.getQID()<<", "<<sh_cand.getChiValue()<<", "<<sh_cand.getQDeg()<<", "<<sh_cand.parent<<")\n";
					#endif // DEBUG
			...
			for(; it!=qry_neigh.end(); it++)  // For each neighbor of qid
			{
				...
				for(; target_neigh!=neighID.end(); target_neigh++)
				{
					...
					vp sh_vp(*it, *target_neigh, graph[*target_neigh]->getChiValue(*it), qryG.at(*it)->getDegree());
					#ifdef DEBUG
						sh_vp.parent = cand->getID();
						cout<<"SH PUSH: ("<<sh_vp.getVID()<<", "<<sh_vp.getQID()<<", "<<sh_vp.getChiValue()<<", "<<sh_vp.getQDeg()<<", "<<sh_vp.parent<<")"<<endl;
					#endif // DEBUG
