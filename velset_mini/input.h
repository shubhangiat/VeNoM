...
class vertex_pair
{
   ...
   public:
	vertex_pair(string qid, string vid, long double chi, unsigned int qdeg) : qID(qid), vID(vid), chisq(chi), qdegree(qdeg)	{}// Construct vertex pair
	~vertex_pair() {}	// Deallocate space

	long double getChiValue() const { return chisq; }
	unsigned int get_qDegree() const { return qdegree; }
	string getVID() const { return vID; }
	string getQID() const { return qID; }
	void print() const { cout<<"("<<vID<<", "<<qID<<", "<<chisq<<", "<<qdegree<<")"; }
	#ifdef DEBUG
		string parent;
	#endif	// DEBUG
   ...
   public:
	bool operator() (vp a, vp b)
	{
		...
		if(a_val == b_val)
		{
			...
			return (a_val > b_val);		// Prefer higher degree
		}

		return (a_val > b_val);		// false => current position is okay, true => swap is required
	}
};
